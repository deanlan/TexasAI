...
from texasgpt.component import BaseComponent, ComponentType, SystemApp

texasgpt_TRACER_SPAN_ID = "DB-GPT-Trace-Span-Id"
...
def _parse_span_id(body: Any) -> Optional[str]:
    ...
    from texasgpt._private.pydantic import BaseModel, model_to_dict

    span_id: Optional[str] = None
    if isinstance(body, Request):
        span_id = body.headers.get(texasgpt_TRACER_SPAN_ID)
    elif isinstance(body, dict):
        span_id = body.get(texasgpt_TRACER_SPAN_ID) or body.get("span_id")
    elif isinstance(body, BaseModel):
        dict_body = model_to_dict(body)
        span_id = dict_body.get(texasgpt_TRACER_SPAN_ID) or dict_body.get("span_id")
    if not span_id:
        return None
    else:
